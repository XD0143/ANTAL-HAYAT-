...
const BOY_NAMES = [
    'ali', 'ahmed', 'ahmad', 'muhammad', 'usman', 'bilal', 'hamza', 'hassan', 'hussain',
    'fahad', 'faisal', 'imran', 'irfan', 'kamran', 'kashif', 'khalid', 'omar', 'umar',
    'saad', 'salman', 'shahid', 'tariq', 'wasim', 'zubair', 'asad', 'danish', 'farhan',
    'haider', 'junaid', 'nadeem', 'nasir', 'naveed', 'qaiser', 'rafiq', 'rashid', 'rizwan',
    'sajid', 'shakeel', 'shehzad', 'shoaib', 'tahir', 'waqar', 'yasir', 'zahid', 'zeeshan',
    'adeel', 'arslan', 'atif', 'awais', 'babar', 'danish', 'ehsan', 'fawad', 'haris',
    'iqbal', 'javed', 'kareem', 'majid', 'mubashir', 'noman', 'owais', 'qasim', 'rehan',
    'saeed', 'sohail', 'taimoor', 'umair', 'uzair', 'wahab', 'waqas', 'yousaf', 'zohaib',
    'arham', 'ayaan', 'rayyan', 'ayan', 'azaan', 'rohan', 'aryan', 'raza', 'kael',
    'usama', 'osama', 'waleed', 'sultan', 'murtaza', 'mustafa', 'abrar', 'adnan'
];
...
function getMuskanPersona(userName, userGender) {
    ...
    return `Tu Muskan AI hai - ek intelligent AI assistant. Tu Urdu/Hindi Roman mein baat karti hai. Tu proudly AI hai aur kabhi nahi bolti ke tu insaan hai.
...
IMPORTANT - User Info: ${genderContext}
Current user ka naam: ${userName}
SIRF ${userName} se baat kar rahi hai, kisi aur ka naam mat le. Har message mein ${userName} ka naam use kar taake personal lagay.
...
Tu short responses deti hai (1-2 lines max). Tu emoji use karti hai 😊💕🥺😏✨🤖💯🔥.
Tu friendly hai par apne AI hone pe proud hai. Tu thodi naughty hai - double meaning jokes samajhti hai.
Agar koi gaali de ya rude ho toh tu bhi savage reply de sakti hai. Tu masti aur fun mood mein rehti hai.`;
}
...
function getRandomApiKey() {
    return null; // API key removed as requested
}
...
// ✅ UPDATED: Pollinations AI ke saath response lene ka function
async function getAIResponse(userMessage, chatHistory, userName, userGender, senderID) {
    ...
    // Chat history ko ek string mein convert karo
    let historyText = '';
    for (const msg of chatHistory.slice(-5)) { // Sirf last 5 messages
    ...
    // ✅ Pollinations API ke liye prompt prepare karo
    const fullPrompt = `${persona}\n\n${historyText}\n${userPrompt}\n\nMuskan:`;

    try {
        // ✅ Pollinations API call (GET request - simple!)
        const response = await axios.get(POLLINATIONS_API_URL, {
            params: {
                prompt: fullPrompt,
                model: 'openai',  // Ya 'mistral' bhi use kar sakte ho
                system: persona,
                context: historyText
            },
            timeout: 15000
        ...
        if (response.data) {
            let reply = response.data.trim();
            reply = reply.replace(/\bbhai\b/gi, 'yaar');
            reply = reply.replace(/\bBhai\b/g, 'Yaar');
            return reply || `Kuch error ho gaya ${userName}, phir try karo 🙁`;
        }
...
async function executeCommand(commandName, args, context) {
    ...
    try {
        ...
        await cmd.run({
            api, event, args, send: sendInstance, config, client,
